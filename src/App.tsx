...
- **Tip**: To get a real, intelligent response from Google Gemini 3.5, please save your API key inside the **Secrets** panel in Google AI Studio!`,
  ...
  return (
    <div className="relative min-h-screen bg-[#070A13] text-[#E0E6F2] font-sans flex flex-col selection:bg-indigo-500/40 overflow-x-hidden">
      ...
      >
        {/* Logo & Title Section with Staggered Animation */}
        <motion.div 
        ...
        >
          <motion.div 
            className="h-10 w-10 rounded-xl bg-gradient-to-tr from-indigo-500 via-purple-600 to-cyan-400 flex items-center justify-center p-[1.5px] shadow-[0_0_15px_rgba(99,102,241,0.4)] hover:scale-110 transition-transform duration-300"
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.35 }}>
            <div className="flex items-center gap-2">
              ...
              <motion.span 
                className="hidden sm:inline-block px-2 py-0.5 text-[10px] font-mono font-bold uppercase rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 shadow-[0_0_8px_rgba(99,102,241,0.2)]"
        ...
        {/* Brand new Top Navigation Bar with Staggered Button Animation */}
        <motion.nav 
          className="flex items-center gap-1 bg-[#0F1322]/80 border border-white/5 p-1 rounded-xl shadow-inner"
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.4, ease: "easeOut" }}
        >
          {[
            { tab: "study", label: "AI Coach", icon: MessageSquare },
            { tab: "terminal", label: "ADK Terminal", icon: Terminal },
            { tab: "multiagent", label: "Multi-Agent team", icon: Users },
            { tab: "quiz", label: "Unit 5 Knowledge Quiz", icon: Award }
          ].map((item, idx) => (
            <motion.button
              key={item.tab}
              ...
              whileTap={{ scale: 0.95 }}
            >
              {item.icon === MessageSquare && <MessageSquare className="h-3.5 w-3.5" />}
              {item.icon === Terminal && <Terminal className="h-3.5 w-3.5" />}
              {item.icon === Users && <Users className="h-3.5 w-3.5 text-cyan-400 animate-pulse" />}
              {item.icon === Award && <Award className="h-3.5 w-3.5 text-cyan-400 animate-pulse" />}
              <span className={item.tab === "quiz" ? "font-extrabold tracking-wider bg-gradient-to-r from-cyan-300 to-indigo-300 bg-clip-text text-transparent" : ""}>
                {item.label}
              </span>
            </motion.button>
          ))}
        ...
        {/* Global Progress Hub with Slide-in Animation */}
        <motion.div 
          className="flex items-center gap-4 bg-[#0F172A]/50 px-4 py-2 rounded-xl border border-white/5 backdrop-blur-sm shadow-inner"
          initial={{ opacity: 0, x: 30 }}
          animate={{ opacity: 1, x: 0 }}
        ...
        >
          <div className="flex flex-col items-end">
            ...
            <motion.span 
              className="text-sm font-black text-transparent bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          <motion.div 
            className="w-24 bg-gray-900/80 h-2 rounded-full overflow-hidden border border-white/5"
